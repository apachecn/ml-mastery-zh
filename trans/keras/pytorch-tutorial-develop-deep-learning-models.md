...
这可以通过对测试数据集使用*数据加载器*并收集测试集的预测，然后将预测与测试集的期望值进行比较并计算表现指标来实现。
...
我们将使用[标签编码器](https://Sklearn.org/stable/modules/generated/sklearn.preprocessing.LabelEncoder.html)将字符串标签编码为整数值 0 和 1。该模型将适用于 67%的数据，剩余的 33%将用于评估，使用 [train_test_split()功能](https://Sklearn.org/stable/modules/generated/sklearn.model_selection.train_test_split.html)进行分割。
