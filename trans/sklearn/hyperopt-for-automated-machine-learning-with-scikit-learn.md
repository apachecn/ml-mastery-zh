...
> 我们的方法是展示如何根据超参数计算表现指标(例如，验证示例的分类精确率)的底层表达式图，这些超参数不仅控制如何应用各个处理步骤，甚至还控制包括哪些处理步骤。
...
在运行结束时，通过调用 *score()* 函数，可以在新数据上评估表现最佳的模型。
...
在运行结束时，我们将报告模型在保持数据集上的表现，并总结表现最好的管道。
...
在运行结束时，在保持数据集上评估表现最佳的模型，并打印发现的管道供以后使用。
