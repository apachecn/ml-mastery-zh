...
这些问题可以通过执行**灵敏度分析**来量化数据集大小和模型表现之间的关系来解决。一旦计算出来，我们就可以解释分析的结果，并决定有多少数据是足够的，以及一个数据集可以有多小，以有效地估计较大数据集的表现。
...
解决这个问题的一种方法是执行[灵敏度分析](https://en.wikipedia.org/wiki/Sensitivity_analysis)，并发现模型在数据集上的表现如何随数据的多少而变化。
