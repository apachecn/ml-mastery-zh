...
具体地，`n_epochs`参数在 _run（）_ 函数中被设置为`1000`。
...
具体地，在 _run（）_ 函数中将`n_epochs`参数设置为 2000。
...
具体地，在 _run（）_ 函数中将`n_epochs`参数设置为 4000。
...
对 _run（）_ 函数中的`n_batch`参数进行了此更改;例如：
...
我们可以在 _run（）_ 函数中更改`n_batch`参数;例如：
...
我们可以通过改变 _run（）_ 函数中的`n_neurons`变量来实现。
...
我们可以通过在 _run（）_ 函数中设置`n_neurons`变量来实现。
