...
IQR可用于通过定义样本值的限制来识别异常值，这些样本值是IQR低于第25百分位数或高于第75百分位数的因子`k`。因子`k`的共同值是值1.5。当在盒子和须状图的背景下描述时，因子k为3或更大可用于识别极端异常值或“_远离_”的值。
...
首先运行该示例打印已识别的第25和第75百分位数以及计算的IQR。打印所识别的异常值的数量，然后是非异常值观察的数量。
