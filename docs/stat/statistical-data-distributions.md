...
_x_ 是来自高斯分布的观察值，_平均值_是`x`的平均值，S是标准差，`n`是观察总数。得到的观察结果形成了（ _n - 1_ ）自由度的t观察。

实际上，如果在计算统计量时需要t分布的值，那么自由度的数量可能是 _n - 1_ ，其中`n`是从高斯分布中抽取样本的大小。
...
下面的示例使用-5到5和（10,000 - 1）自由度的样本空间创建t分布。
...
_chi_ 是具有卡方分布的观察值，`x`是从高斯分布中得出的观察值，`k`是 _x的数量_观察也是卡方分布的自由度数。
