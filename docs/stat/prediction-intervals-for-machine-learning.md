...
我们不知道系数`b0`和`b1`的真实值。我们也不知道真实的群体参数，例如`x`或`y`的平均值和标准偏差。必须估计所有这些元素，这为模型的使用带来了不确定性，以便进行预测。
...
_y_ 是预测值，`z`是高斯分布的临界值（例如，95％间隔为1.96），`sigma`是标准偏差预测的分布。
...
_stdev_ 是预测分布的标准偏差的无偏估计，`n`是所做的总预测，而 _e（i）_是之间的差异。 ith预测和实际价值。
...
然后将系数与来自数据集的输入一起使用以进行预测。得到的输入和预测的`y`值被绘制为数据集的散点图顶部的线。
