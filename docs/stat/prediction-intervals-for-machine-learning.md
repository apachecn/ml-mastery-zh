...
其中_是_是由训练模型对给定输入数据 _X_ 进行的估计结果或预测。
...
在使用回归模型进行预测或预测时，最常使用预测区间，其中正在预测数量。
...
在预测建模中，置信区间可用于量化模型的估计技能的不确定性，而预测区间可用于量化单个预测的不确定性。
...
在非线性回归算法（如人工神经网络）的情况下，它更具挑战性，需要选择和实施专门技术。可以使用诸如自举重采样方法之类的一般技术，但计算上计算成本很高。
...
是预测的_， _b0_ 和 _b1_ 是根据训练数据估计的模型的系数， _x_ 是输入变量。_

我们不知道系数 _b0_ 和 _b1_ 的真实值。我们也不知道真实的群体参数，例如 _x_ 或 _y_ 的平均值和标准偏差。必须估计所有这些元素，这为模型的使用带来了不确定性，以便进行预测。

我们可以做出一些假设，例如 _x_ 和 _y_ 的分布，并且由模型做出的预测误差（称为残差）是高斯分布。
...
_y_ 是预测值， _z_ 是高斯分布的临界值（例如，95％间隔为1.96）， _sigma_ 是标准偏差预测的分布。
...
_stdev_ 是预测分布的标准偏差的无偏估计， _n_ 是所做的总预测，而 _e（i）_是之间的差异。 ith预测和实际价值。

## 工作示例

让我们以线性回归预测区间为例，用一个实例说明。

首先，让我们定义一个简单的双变量数据集，其中输出变量（ _y_ ）取决于具有一些高斯噪声的输入变量（ _x_ ）。
...
接下来，我们可以开发一个简单的线性回归，给出输入变量 _x_ ，将预测 _y_ 变量。我们可以使用 [linregress（）SciPy函数](https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.linregress.html)拟合模型并返回模型的 _b0_ 和 _b1_ 系数。
...
我们可以使用系数来计算每个输入变量的预测 _y_ 值，称为 _yhat_ 。结果点将形成表示学习关系的线。
...
然后将系数与来自数据集的输入一起使用以进行预测。得到的输入和预测的 _y_ 值被绘制为数据集的散点图顶部的线。
...
我们现在准备用我们的简单线性回归模型进行预测并添加预测间隔。

我们将像以前一样适应模型。这次我们将从数据集中取一个样本来演示预测间隔。我们将使用输入进行预测，计算预测的预测间隔，并将预测和间隔与已知预期值进行比较。
