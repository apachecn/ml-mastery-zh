...
**注意**：我建议使用这些SciPy函数计算应用程序的Student t检验，如果它们合适的话。库实现将更快，更不容易出错。我只建议您出于学习目的自行实施测试，或者在需要修改测试版本的情况下。
...
我们可以使用内置的SciPy函数 _ttest_ind（）_计算这些样本的t检验。这将为我们提供t统计值和要比较的p值，以确保我们正确地实施了测试。
...
我们可以看到t统计量和p值都与SciPy函数的输出相匹配。测试似乎正确实施。
...
计算出的t统计量和p值与我们对SciPy库实现的期望相匹配。这表明实施是正确的。
