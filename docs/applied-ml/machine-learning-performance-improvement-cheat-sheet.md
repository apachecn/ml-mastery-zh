...
*   **重构你的问题**：_你能改变你正在解决的预测问题的类型吗？_ 将您的数据重新构建为回归，二元或多类分类，时间序列，异常检测，评级，推荐等类型问题。
...
*   **评估指标**。 _用什么指标来评估预测技巧？_ 使用最能捕获问题和域要求的指标。它可能不是分类准确性。
...
*   **替代实施**。 _该算法的其他实现是什么？_ 也许该方法的替代实现可以在相同数据上获得更好的结果。每种算法都有许多必须由算法实现者做出的微决策。其中一些决定可能会影响您的问题的技能。
*   **算法扩展**。 _算法的常见扩展是什么？_ 也许您可以通过评估方法的常用或标准扩展来提升表现。这可能需要实施工作。
