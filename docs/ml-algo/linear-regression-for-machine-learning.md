...
在本节中，我们将简要介绍准备线性回归模型的四种技术。这不足以从头开始实现它们，但足以让人了解所涉及的计算和权衡。
...
当输入值存在共线性且普通最小二乘法会过度拟合训练数据时，这些方法可以有效使用。

既然您已经了解了一些在线性回归模型中学习系数的技术，那么让我们看一下如何使用模型对新数据进行预测。

## 用线性回归进行预测

如果表示是一个线性方程，那么进行预测就像解决一组特定输入的方程一样简单。
...
既然我们已经知道如何在学习线性回归模型的情况下进行预测，那么让我们看看准备数据的一些经验法则，以充分利用这种类型的模型。
...
*   **删除共线性**。当您具有高度相关的输入变量时，线性回归将过度拟合您的数据。考虑计算输入数据的成对相关性并删除最相关的数据。
