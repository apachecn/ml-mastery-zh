...
该模型很简单，但考虑到训练它所需的大量数据，调整模型中的无数设计决策以获得最佳性能问题可能实际上难以处理。值得庆幸的是，研究科学家已经使用 Google 规模的硬件为我们完成这项工作，并提供了一套启发式方法，用于如何配置编码器 - 解码器模型进行神经机器翻译和一般的序列预测。
...
通常，深层网络被认为比浅层网络具有更好的性能。

关键是要在网络深度，模型技能和培训时间之间找到平衡点。这是因为如果对技能的好处很小，我们通常没有无限的资源来训练非常深的网络。

作者探讨了编码器和解码器模型的深度以及对模型技能的影响。

当涉及编码器时，发现深度对技能没有显着影响，更令人惊讶的是，1 层单向模型仅比 4 层单向配置略差。双层双向编码器的性能略优于其他测试配置。
...
> ...我们确实表明，通过仔细的超参数调整和良好的初始化，可以在标准 WMT 基准测试中实现最先进的性能
