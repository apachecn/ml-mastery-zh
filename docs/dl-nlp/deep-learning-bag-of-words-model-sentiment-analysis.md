...
该数据已用于一些相关的自然语言处理任务。对于分类，经典模型（例如支持向量机）对数据的性能在高 70％至低 80％（例如 78％-82％）的范围内。
...
通过使用评论的文件名可以轻松实现拆分，其中评论为 000 至 899 的评论用于培训数据，而评论为 900 以上的评论用于测试模型。
...
我们还需要所有培训和测试审核数据的类标签。我们确定性地加载并编码了这些评论（否定，然后是正面），因此我们可以直接指定标签，如下所示：
...
将使用梯度下降的有效 [Adam 实现](http://machinelearningmastery.com/adam-optimization-algorithm-for-deep-learning/)和二进制交叉熵损失函数来训练网络，适合于二元分类问题。在培训和评估模型时，我们将跟踪准确性。
...
最后，一旦训练了模型，我们就可以通过在测试数据集中进行预测并打印精度来评估其性能。
...
我们可以看到'freq'配置的分布是紧张的，这是令人鼓舞的，因为它也表现良好。此外，我们可以看到“二进制”通过适度的传播实现了最佳结果，可能是此数据集的首选方法。
...
*   **调整网络拓扑**。探索其他网络拓扑，例如更深或更广的网络。也许您可以通过更适合的网络获得更好的性能。
*   **使用正则化**。探索正规化技术的使用，例如丢失。也许您可以延迟模型的收敛并实现更好的测试集性能。
