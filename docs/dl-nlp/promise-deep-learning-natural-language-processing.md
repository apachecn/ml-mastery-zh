...
在自然语言处理领域深度学习的承诺是模型的更好性能，这些模型可能需要更多数据但是培训和操作的语言专业知识更少。
...
1.  **直接替换模型的承诺**。也就是说，深度学习方法可以放入现有的自然语言系统中作为可以实现相称或更好性能的替代模型。
...
5.  **端到端模型的承诺**。也就是说，大型端到端深度学习模型可以适应自然语言问题，提供更通用，性能更好的方法。
...
自然语言处理中深度学习的第一个承诺是能够用能够学习和利用非线性关系的性能更好的模型替换现有的线性模型。
...
> 一般而言，我们手动设计的功能往往过于规范，不完整，需要很长时间进行设计和验证，并且只能在一天结束时达到一定的性能水平。学习的功能易于适应，快速训练，他们可以继续学习，以便他们达到我们以前能够达到的更好的性能水平。
...
除了这些模型的改进性能之外，这对于开发的速度和简单性都是期望的。
...
迄今为止，深度学习方法已经在自然语言处理的更广泛的问题中进行了评估，并在一小部分上取得了成功，其中成功表明性能或能力达到或高于之前使用其他方法的可能性。
