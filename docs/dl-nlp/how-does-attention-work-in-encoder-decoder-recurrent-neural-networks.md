...
> 我们提出了一种新颖的神经网络体系结构，它学习将可变长度序列编码成固定长度的矢量表示，并将给定的固定长度矢量表示解码回可变长度序列。
...
提出注意作为对编码器 - 解码器模型的限制的解决方案，该编码器 - 解码器模型将输入序列编码为一个固定长度矢量，从该矢量解码每个输出时间步长。在解码长序列时，这个问题被认为是更多的问题。

> 这种编码器 - 解码器方法的潜在问题是神经网络需要能够将源句子的所有必要信息压缩成固定长度的矢量。这可能使神经网络难以处理长句，特别是那些比训练语料库中的句子长的句子。
...
注意模型不是将输入序列编码成单个固定的上下文矢量，而是开发一个特定于每个输出时间步骤过滤的上下文矢量。
...
在本节中，我们将通过一个小的工作示例来注意具体。具体来说，我们将使用未矢量化的术语逐步完成计算。
...
在编码器 - 解码器模型中，输入将被编码为单个固定长度矢量。这是最后一个时间步的编码器模型的输出。
