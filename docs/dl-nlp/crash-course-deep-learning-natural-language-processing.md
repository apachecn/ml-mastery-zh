...
深度学习的一个特性是这些类型的模型的性能通过增加其深度或代表能力来训练它们的更多示例而得到改善。
...
*   **端到端模型的承诺**。也就是说，大型端到端深度学习模型可以适应自然语言问题，提供更通用，性能更好的方法。
...
一般来说，我们指的是将原始文本转换为我们可以建模为“标记化”的东西的过程，其中我们留下了单词列表或“标记”。

我们可以手动开发 Python 代码来清理文本，并且这通常是一种很好的方法，因为每个文本数据集必须以独特的方式进行标记化。
...
安装后，您可以使用 API​​来标记文本。例如，下面的代码段将加载并标记化 ASCII 文本文件。
...
您的任务是在 Project Gutenberg 网站上找到一本免费的经典书籍，下载书籍的 ASCII 版本并将文本标记化并将结果保存到新文件中。探索手动和 NLTK 方法的加分点。
...
如上所述，编码器必须在源文档上进行训练，然后可用于对将来的训练数据，测试数据和任何其他数据进行编码。 API 还具有在对单词进行编码之前执行基本标记化的优点。
...
Gensim 提供了 word2vec 算法的实现，该算法是在 Google 开发的，用于快速培训来自文本文档的字嵌入表示，
