...
在这篇文章中，您将了解如何使用 Keras 深度学习库开发深度学习模型，以便在 Python 中使用 MNIST 手写数字识别任务实现近乎最先进的性能。
...
训练数据集被构造为实例，图像宽度和图像高度的三维阵列。对于多层感知器模型，我们必须将图像缩小为像素矢量。在这种情况下，28×28 大小的图像将是 784 像素输入值。
...
运行该示例可在每个纪元和最终分类错误率的训练和验证数据集上打印准确度。
